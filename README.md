...
- Bundled reference loader now also reads optional Basic Set skill data from `data/basic-set-skills.reference-summaries.json`.
...
- Reference helpers:
  ...
  - Module authors can ship local reference summaries in `data/reference-summaries.json` and optional expansion packs in `data/basic-set-skills.reference-summaries.json`, `data/martial-arts-techniques.reference-summaries.json`, `data/martial-arts-combat.reference-summaries.json`, and `data/magic.reference-summaries.json`; popup matching is exact `name + type` first, then exact `name`, and the popup displays **Author Summary**, optional notes, and optional source/page metadata.
