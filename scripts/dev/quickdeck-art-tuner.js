...
const STRUCTURAL_WRAPPER_CLASS_SET = new Set(STRUCTURAL_WRAPPER_CLASSES);
const PRESETS = [
...
function renderPanel(options = {}) {
  ensureChrome();
  const searchWasFocused = options.restoreSearchFocus || document.activeElement?.dataset?.action === "target-search";
  const searchSelectionStart = searchWasFocused ? document.activeElement?.selectionStart ?? state.targetSearch.length : state.targetSearch.length;
  const searchSelectionEnd = searchWasFocused ? document.activeElement?.selectionEnd ?? state.targetSearch.length : state.targetSearch.length;
  ...
  const visibleTargets = getVisibleBrowserTargets(browserTargets);
  state.panel.innerHTML = `
    <div class="qd-art-tuner-header" style="position: sticky; top: 0; margin: 0 -10px 8px; padding: 8px 10px; cursor: grab; color: #1b1208; background: #d4a84b; font-weight: 700;">QuickDeck Art/Layout Tuner <span style="font-weight:400">drag header</span></div>
    <div style="display:grid; gap:6px;">
      <div><strong>Selected:</strong> ${escapeHtml(selectedLabel)}</div>
      <div style="word-break: break-word;"><strong>Selector:</strong> ${escapeHtml(selector)}</div>
      <div><strong>Adjust:</strong> ${escapeHtml(values)}</div>
      <div class="qd-art-tuner-actions" style="display:flex; flex-wrap:wrap; gap:4px;">${buttonHtml("copy", "Copy CSS")}${buttonHtml("copy-selected", "Copy selected")}${buttonHtml("reset-selected", "Reset selected")}${buttonHtml("reset-all", "Reset all")} ${buttonHtml("off", "Off")}${buttonHtml("parent", "Parent")}${buttonHtml("same-class", "All same class")}${buttonHtml("status", "Status")}</div>
      <div><strong>Move</strong></div>
      <div class="qd-art-tuner-nudges" style="display:flex; flex-wrap:wrap; gap:4px;">${nudgeButtonHtml("X -10", -10, 0, 0, 0)}${nudgeButtonHtml("X -1", -1, 0, 0, 0)}${nudgeButtonHtml("X +1", 1, 0, 0, 0)}${nudgeButtonHtml("X +10", 10, 0, 0, 0)} ${nudgeButtonHtml("Y -10", 0, -10, 0, 0)}${nudgeButtonHtml("Y -1", 0, -1, 0, 0)}${nudgeButtonHtml("Y +1", 0, 1, 0, 0)}${nudgeButtonHtml("Y +10", 0, 10, 0, 0)}</div>
      <div><strong>Resize</strong></div>
      <div class="qd-art-tuner-nudges" style="display:flex; flex-wrap:wrap; gap:4px;">${nudgeButtonHtml("W -10", 0, 0, -10, 0)}${nudgeButtonHtml("W -1", 0, 0, -1, 0)}${nudgeButtonHtml("W +1", 0, 0, 1, 0)}${nudgeButtonHtml("W +10", 0, 0, 10, 0)} ${nudgeButtonHtml("H -10", 0, 0, 0, -10)}${nudgeButtonHtml("H -1", 0, 0, 0, -1)}${nudgeButtonHtml("H +1", 0, 0, 0, 1)}${nudgeButtonHtml("H +10", 0, 0, 0, 10)}</div>
      <div><strong>Target browser</strong></div>
      <label style="display:grid; gap:2px;">Category
        <select data-action="target-category">${TARGET_CATEGORIES.map((category) => `<option value="${escapeAttr(category)}"${category === state.targetCategory ? " selected" : ""}>${escapeHtml(category)}</option>`).join("")}</select>
      </label>
      <label style="display:grid; gap:2px;">Search targets...
        <input type="search" data-action="target-search" value="${escapeAttr(state.targetSearch)}" placeholder="Search targets..." autocomplete="off" style="box-sizing:border-box; width:100%;">
      </label>
      <label style="display:flex; align-items:center; gap:6px;"><input type="checkbox" data-action="show-structural-wrappers"${state.showStructuralWrappers ? " checked" : ""}> Show structural wrappers</label>
      <div class="qd-art-tuner-targets" style="display:grid; gap:3px; max-height: 170px; overflow:auto; padding-right: 2px;">${visibleTargets.length ? visibleTargets.map((target) => targetButtonHtml(target)).join("") : `No matching QuickDeck elements found.`}</div>
      <div><strong>Presets</strong></div>
      <div class="qd-art-tuner-presets" style="display:flex; flex-wrap:wrap; gap:4px;">${PRESETS.map((preset) => buttonHtml("preset", preset.label, preset.label)).join("")}</div>
      <small>Right-click selects a specific element. Left-drag selected outline moves; Alt+drag resizes. Ctrl+right-click selects parent.</small>
    </div>`;
  ...
  if (searchWasFocused) restoreSearchInputFocus(searchSelectionStart, searchSelectionEnd);
}
...
function restoreSearchInputFocus(selectionStart, selectionEnd) {
  ...
  const start = Math.min(Number(selectionStart) || 0, input.value.length);
  const end = Math.min(Number(selectionEnd) || start, input.value.length);
  input.setSelectionRange?.(start, end);
}

function getVisibleBrowserTargets(browserTargets) {
  ...
  return browserTargets
    ...
    .filter((target) => state.showStructuralWrappers || !target.structuralWrapper)
    .filter((target) => !query || target.searchText.includes(query));
}

function buttonHtml(action, label, value = "") {
  return `<button type="button" data-action="${escapeAttr(action)}"${value ? ` data-value="${escapeAttr(value)}"` : ""}>${escapeHtml(label)}</button>`;
}

function nudgeButtonHtml(label, dx, dy, dw, dh) {
  return `<button type="button" data-action="nudge" data-dx="${dx}" data-dy="${dy}" data-dw="${dw}" data-dh="${dh}">${escapeHtml(label)}</button>`;
}

function targetButtonHtml(target) {
  return `<button type="button" data-action="select-target" data-index="${target.index}" title="${escapeAttr(target.selector)}" style="text-align:left; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${escapeHtml(target.label)}</button>`;
}
...
function onNudgeAction(event) {
  ...
  adjustSelected({ dx: numberFromDataset(button.dataset.dx), dy: numberFromDataset(button.dataset.dy), dw: numberFromDataset(button.dataset.dw), dh: numberFromDataset(button.dataset.dh) });
}
...
function onTargetSearchInput(event) {
  ...
  saveState();
  renderPanel({ restoreSearchFocus: true });
}
...
function loadState() {
  try {
    ...
    if (parsed.selected?.selector) {
      const selectedSelector = scopeSelector(parsed.selected.selector);
      state.selected = {
        label: parsed.selected.label || selectedSelector,
        selector: selectedSelector,
        key: makeKey(selectedSelector, parsed.selected.pseudo),
        pseudo: parsed.selected.pseudo || "",
        baseWidth: parsed.selected.baseWidth || "",
        baseHeight: parsed.selected.baseHeight || "",
        classSelector: parsed.selected.classSelector || getClassSelectorFromSelector(selectedSelector)
      };
    }
...
function saveState() {
  ...
  window.localStorage?.setItem(STORAGE_KEY, JSON.stringify({
    ...
    selected: state.selected ? {
      ...
      classSelector: state.selected.classSelector || ""
    } : null,
...
function normalizeEntry(entry) {
  return { label: entry.label || entry.selector, selector: scopeSelector(entry.selector), pseudo: entry.pseudo || "", baseWidth: entry.baseWidth || "", baseHeight: entry.baseHeight || "", x: Number(entry.x) || 0, y: Number(entry.y) || 0, w: Number(entry.w) || 0, h: Number(entry.h) || 0 };
}
...
function clampPanelPosition(left, top) {
  ...
  return { left: Math.min(Math.max(0, Math.round(Number(left) || 0)), maxLeft), top: Math.min(Math.max(0, Math.round(Number(top) || 0)), maxTop) };
}
...
function scanTargets() {
  ...
  return Array.from(candidates).map((element, index) => {
    ...
    return {
      ...
      label,
      classList,
      structuralWrapper: isStructuralWrapper(element),
      searchText: buildTargetSearchText({ element, label, selector, category, classList })
    };
  });
}
...
function isStructuralWrapper(targetOrElement) {
  const element = targetOrElement?.element || targetOrElement;
  if (!element || typeof Element === "undefined" || !(element instanceof Element)) return false;
  return Array.from(element.classList || []).some((className) => STRUCTURAL_WRAPPER_CLASS_SET.has(className));
}

function buildTargetSearchText({ element, label, selector, category, classList }) {
...
function getOverlayElement(target) {
  const overlay = document.querySelector(OVERLAY_SELECTOR);
  if (!target || typeof Element === "undefined" || !(target instanceof Element) || !overlay?.contains(target)) return null;
  return target;
}

function buildSelector(element) {
  const overlay = document.querySelector(OVERLAY_SELECTOR);
  if (!element || typeof Element === "undefined" || !(element instanceof Element) || !overlay?.contains(element)) return OVERLAY_SELECTOR;
